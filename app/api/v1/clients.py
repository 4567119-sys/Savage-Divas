...
@router.post("/bootstrap", response_model=ClientResponse)
def bootstrap_client(
    client_data: ClientCreate,
    db: Session = Depends(get_db),
):
    """Create the demo/client record if it does not exist, otherwise return it."""
    existing_client = db.query(Client).filter(Client.email == client_data.email).first()
    if existing_client:
        return existing_client
    client = Client(id=str(uuid4()), **client_data.model_dump())
    db.add(client)
    db.commit()
    db.refresh(client)
    return client
