(function () {
  ...
  const DOM = {
    ...
    callConfirmModal: $('callConfirmModal'),
    btnCloseCallConfirm: $('btnCloseCallConfirm'),
    btnCancelCall: $('btnCancelCall'),
    btnEmergencyCallFooter: $('btnEmergencyCallFooter'),
  ...
  function getBestVoice() {
    ...
    let candidates = voices.filter((v) => v.lang.toLowerCase().startsWith(code.slice(0, 2)));
  ...
  function openGeneric(type) {
    ...
    DOM.genericTitle.textContent = copy.title;
    DOM.genericLead.textContent = copy.lead;
  ...
  async function ensureBackendClient() {
    const u = state.currentUser || DEFAULT_USER;
    return apiJson('/clients/bootstrap', {
      method: 'POST',
      body: JSON.stringify({
        full_name: u.name,
        ...
        accessibility_mode: state.isHighContrast
      })
    });
  }

  async function submitBackendCase() {
    const client = await ensureBackendClient();
    const created = await apiJson('/cases', {
      method: 'POST',
      body: JSON.stringify({
        client_id: client.id,
        case_type: 'ACCIDENT',
        title: `Vehicle accident - ${state.formData.location || 'Reported incident'}`
      })
    });

    const docs = Object.values(state.formData.documents || {}).filter(Boolean);
    for (const doc of docs) {
      if (!doc.name) continue;
      const body = new FormData();
      if (doc.file) body.append('file', doc.file, doc.name);
      else {
        const placeholder = new Blob([`Demo document reference: ${doc.name}`], { type: 'text/plain' });
        body.append('file', placeholder, doc.name);
      }
      body.append('file_type', doc.type || 'supporting_document');
      await fetch(`${API_BASE}/cases/${created.id}/documents`, { method: 'POST', body });
    }

    if (docs.length) {
      await apiJson('/ai/ingest', {
        method: 'POST',
        body: JSON.stringify({
          case_id: created.id,
          document_id: null,
          doc_type_detected: 'SUPPORTING_DOCUMENT',
          overall_confidence: 0.92,
          extracted_fields: {
            incident_location: state.formData.location,
            incident_date: state.formData.date,
            police_case_number: state.formData.caseNumber,
            police_station: state.formData.station,
            investigating_officer: state.formData.officer
          },
          field_confidences: {},
          flagged_anomalies: []
        })
    ...
    const progress = await apiJson(`/cases/${created.id}/progress`);
  ...
  async function submitAccidentReport() {
    ...
    if (online) {
      try {
        const result = await submitBackendCase();
        const localRef = ref;
        ref = `RS-${result.created.id.slice(0, 6).toUpperCase()}`;
        const claim = state.claims.find((c) => c.id === localRef);
        if (claim) {
          claim.backendCaseId = result.created.id;
          claim.id = ref;
        }
        announce('Request submitted to the Royal Square Financial backend.');
      } catch (error) {
        console.error(error);
        const claim = state.claims.find((c) => c.id === ref);
        if (claim) {
          claim.status = 'waiting_to_sync';
          claim.pipeline = 'submitted';
        }
        announce('The request was saved on this device because the backend could not be reached.');
      }
    }
  ...
  function bindDocUploads() {
    ...
    DOM.fileDamage.addEventListener('change', (e) => {
      ...
      reader.onload = (evt) => {
        state.formData.documents.damage = { name: file.name, url: evt.target.result, file, type: file.type };
  ...
  function bindEvents() {
    ...
    DOM.btnEmergencyCallFooter.addEventListener('click', () => DOM.callConfirmModal.classList.add('active'));
    DOM.btnCloseCallConfirm.addEventListener('click', () => DOM.callConfirmModal.classList.remove('active'));
    DOM.btnCancelCall.addEventListener('click', () => DOM.callConfirmModal.classList.remove('active'));
    ...
    document.addEventListener('keydown', (e) => {
      if (e.key === 'Escape') {
        ...
        DOM.callConfirmModal.classList.remove('active');
        DOM.voicePanel.hidden = true;
      }
    });
